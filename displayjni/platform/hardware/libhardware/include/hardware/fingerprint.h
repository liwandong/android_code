...
#ifdef USE_CDFINGER_MODULE_ID
#define FINGERPRINT_HARDWARE_MODULE_ID "cdfinger.fingerprint"
#else
...
typedef enum fingerprint_msg_type
{
    ...
    // 2018-09-06, add by norz for LG-fido
    FINGERPRINT_MSG_VENDOR_BASE = 1000,
    FINGERPRINT_AUTHENTICATED_EX = FINGERPRINT_MSG_VENDOR_BASE + 0, // FIDO Authenticate msg type
...
typedef enum fingerprint_acquired_info
{
    ...
    FINGERPRINT_ACQUIRED_WAIT_FINGER_INPUT,  // 1001
    FINGERPRINT_ACQUIRED_TOUCH,              // 1002
    FINGERPRINT_ACQUIRED_UNTOUCH,            // 1003
    FINGERPRINT_ACQUIRED_INPUT_TOO_LONG,     // 1004
    FINGERPRINT_ACQUIRED_DUPLICATE_FINGER,   // 1005
    FINGERPRINT_ACQUIRED_DUPLICATE_AREA,     // 1006
    FINGERPRINT_ACQUIRED_SIMULATE_FINGER,    // 1007
    FINGERPRINT_ACQUIRED_SINGLE_CLICK,       // 1008
    FINGERPRINT_ACQUIRED_DOUBLE_CLICK,       // 1009
} fingerprint_acquired_info_t;

typedef enum fingerprint_mmi_test_cmd
{
    MMI_TEST_CMD_SPI = 1,
    MMI_TEST_CMD_INTERRUPT = 2,
    MMI_TEST_CMD_BAD_PIXEL = 3,
    MMI_TEST_CMD_DETECT_FINGER = 4,
} fingerprint_mmi_test_cmd_t;
...
// 2018-09-06, add by norz for LG-fido - start
#define FIDO_UVT_SIZE                                                                                                  \
    288 // Maximum size of encrypted UVT. Plain UVT size is 144 bytes. Size of encrypted UVT is plain UVT size plus
        // maximum 144 bytes.
typedef struct fingerprint_fido_uvt
{
    uint8_t token[FIDO_UVT_SIZE]; // Generated and encrypted UVT by fingerprint.
    uint32_t tokenSize;           // Actual encrypted UVT size.
} fingerprint_fido_uvt_t;

typedef struct fingerprint_authenticated_ex
{
    fingerprint_finger_id_t finger;
    fingerprint_fido_uvt_t uvt;
} fingerprint_authenticated_ex_t;
// 2018-09-06, add by norz for LG-fido - end

typedef struct fingerprint_mmi_test
{
    int cmd;
    int result;
} fingerprint_mmi_test_t;
...
typedef struct fingerprint_msg
{
    ...
    union {
        ...
        fingerprint_mmi_test_t mmi_test;
        // 2018-09-06, add by norz for LG-fido
        fingerprint_authenticated_ex_t authenticated_ex;
    } data;
} fingerprint_msg_t;
...
typedef struct fingerprint_device
{
    ...
    // 2018-09-06, add by norz for LG-fido - start
    /*
     * Authenticates for FIDO operation
     *
     * If flags equals to 0 then result UVT should be encrypted for "csfido" TA
     * If flags equals to 1 then result UVT should be encrypted for "cdcvm" TA
     *
     * Function return: 0 on success
     *                  or a negative number in case of error, generally from the errno.h set.
     */
    int (*authenticate_ex)(struct fingerprint_device *dev, uint32_t gid, const uint8_t *challenge,
                           size_t challengeLength, uint32_t flags);

    /*
     * Cancel pending FIDO authenticate, sending FINGERPRINT_ERROR_CANCELED
     * to all running clients. Switches the HAL state machine back to the idle state.
     *
     * Function return: 0 if cancel request is accepted
     *                  or a negative number in case of error, generally from the errno.h set.
