...
static const char *fingertechclassname = "com/fingertech/displayfingerprint/Fingerprintjni";
...
static jint fingertech_readimgerbuf(JNIEnv *env,jobject obj,jbyteArray buf)
{
 ...
 bytearray = (*env)->GetByteArrayElements(env,buf,NULL);
 if(NULL == bytearray)
 {
  return -1;
 }
 ...
 (*env)->ReleaseByteArrayElements(env,buf,bytearray,0);
 return ret;
}
